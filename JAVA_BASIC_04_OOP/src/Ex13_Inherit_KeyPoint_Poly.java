/*
문제 풀어 보세요 ^^
요구사항
 >> 매장카트 구현하기 입니다 <<
카트 (Cart) >> member field 
카트 매장에 있는 [모든 전자제품]을 담을 수 있다 
카트의 크기는 고정되어 있다 (10개) : 1개  , -> 최대 10개만 저장 가능(배열로 구현)
2개 담을 수 있고 최대 10개까지 담을 수 있다 
고객이 물건을 구매 하면 ... [카트에 담는다]
---------------------------------------
계산대에 가면 전체 계산
계산기능이 필요
summary() 기능 추가해 주세요
당신이 구매한 물건이름 과 가격정보 나열
총 누적금액 계산 출력

hint) 카트 물건을 담는 행위 (Buy() 함수안에서 cart  담는 것을 구현 )
hint) Buyer ..>> summary()  main 함수에서 계산할때  사용합니다

Buyer >> buy() , summary()  >> 카트에 물건 계산 (어떤물건 , 각 가격 >> 당신은 총 얼마 지급)

구매자는 default 금액을 가지고 있고 초기금액을 설정할 수 도 있다

카트 최대 10개까지만 담을 수 있음
딱 10칸만 고정됨 → 배열
계산대에서는 끄집어 내면서 영수증을 만들어줌
카트는 멤버 필드로 만들고 카트는 모든 전자제품을 담을 수 있다.
구매하면 그때 카드에 담기
구매자는 default 금액을 가지고 있고 초기금액을 설정할 수도 있다

*/

// 전자제품의 공통사항(모든 전자제품음 가격과 보너스 포인트를 가지고 있다)
class Product2{
	int price;
	int bonuspoint;
	
	public Product2(int price) {
		this.price = price;
		this.bonuspoint = (int)(this.price / 10.0);
	}
}

class KtTv2 extends Product2{
	KtTv2(){
		super(500);
	}
	
	@Override
	public String toString() {
		return "KtTv";
	}
}

class Audio2 extends Product2{
	Audio2(){
		super(100);
	}
	
	@Override
	public String toString() {
		return "Audio";
	}
}

class NoteBook2 extends Product2{
	NoteBook2(){
		super(150);
	}
	
	@Override
	public String toString() {
		return "NoteBook";
	}
}

// 구매자
class Buyer2{
	
	Product2[] cart; // 카트 만들기
	int money;
	int bonuspoint;
	int index = 0;
	
	// 구매자는 default 금액을 가지고 있고 
	public Buyer2(){
		this(1000);
	}
	
	// 초기금액을 설정할 수도 있다
	public Buyer2(int money){
		cart = new Product2[10]; // 10개를 담을 수 있는 카트 담음
		this.money = money;
	}
	
	// 개선 - 상속 관계에서 부모타입의 참조 변수가 자식 객체의 주소를 가질 수 있다.
	void buy(Product2 n) { // n : TV 정보(가격, 이름) 알고 싶다
		if(this.money < n.price) {
			System.out.println("고객님 잔액이 부족합니다. ^___^, 잔액 : " + this.money);
			return; // 메서드 강제 종료
		}
		
		// 실 구매 행위
		this.money -= n.price;
		this.bonuspoint += n.bonuspoint;
		
		// 카트에 담기
		// 구매할 때마다 카트에 담기
		if(index > 10) {
			System.out.println("카트가 가득 찼습니다.");
			return;
		} 
		
		cart[index++] = n;	
	}
	
	void summery() {

		int totalprice = 0; // 구매한 물건 총액
		int totalbounuspoint = 0; // point 총액
		//String은 성능상 누적이 안좋음
		// 문자열 누적은 StringBUffer
		String productlist="";
		
		// 비효율
//		for(int i = 0;i < cart.length;i++) {
//			if(cart[i] == null) break;
//			totalprice += cart[i].price;
//			totalbounuspoint += cart[i].bonuspoint;
//			productlist += cart[i].toString() + " ";
//		}
		
		// 효율
		for(int i = 0;i < index ;i++) {
			totalprice += cart[i].price;
			totalbounuspoint += cart[i].bonuspoint;
			productlist += cart[i].toString() + " ";
			}
			System.out.println("*******************");
			System.out.println("구매한 물건 총액 : " + totalprice);
			System.out.println("구매한 포인트 총액 : " + totalbounuspoint);
			System.out.println("구매한 물건 목록 : " + productlist);
			System.out.println("*******************");
	}
}

public class Ex13_Inherit_KeyPoint_Poly {
	public static void main(String[] args) {
		Buyer2 b = new Buyer2(5000);
		
		KtTv2 kttv = new KtTv2();
		Audio2 audio = new Audio2();
		NoteBook2 noteBook = new NoteBook2();
		b.buy(kttv);
		b.buy(audio);
		b.buy(kttv);
		b.buy(noteBook);
		b.summery();
	}
	
}
